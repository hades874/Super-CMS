...
export function parseClassData(rawText: string): ParsedClass[] {
  ...
  groupMap.forEach(({ rows, programs }) => {
    ...
    let startTime = getField(row, ['Start Time', 'StartTime', 'start_time']);
    let endTime = '';

    if (rawClassTime.includes('-')) {
      const parts = rawClassTime.split('-').map(p => p.trim());
      if (parts.length >= 2) {
        if (!startTime) startTime = parts[0];
        endTime = parts[1];
      }
    } else if (rawClassTime.toLowerCase().includes('to')) {
        const parts = rawClassTime.split(/to/i).map(p => p.trim());
        if (parts.length >= 2) {
          if (!startTime) startTime = parts[0];
          endTime = parts[1];
        }
